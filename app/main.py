...
if __name__ == '__main__':
    while True:
        ...
        for task in tasks:
            ...
            file_name = 'Contrato RPA nº {rpaID} - {nome_completo}.pdf'.format(rpaID=task.variables['rpaID'].value, nome_completo=task.variables['nome_completo'].value)
            try:
                remote_file = task.variables['documento_assinado'].value
                index = remote_file.rfind('/')
                remote_file_name = remote_file[index+1:]
                
                storage.download(remote_file_name, home + '/outputs/', file_name)
                storage.close()
